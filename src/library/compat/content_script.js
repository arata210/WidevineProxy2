...
function emitAndWaitForResponse(type, data) {
    return new Promise((resolve) => {
        ...
        document.dispatchEvent(new CustomEvent('response', {
            detail: { type, body: data, requestId }
        }));
    });
}
...
class Evaluator {
    static isDASH(text) { return text.includes('<mpd') && text.includes('</mpd>'); }
    static isHLS(text) { return text.includes('#extm3u'); }
    static isHLSMaster(text) { return text.includes('#ext-x-stream-inf'); }
    static isMSS(text) { return text.includes('<smoothstreamingmedia') && text.includes('</smoothstreamingmedia>'); }
    static getManifestType(text) {
        const lower = text.toLowerCase();
        if (this.isDASH(lower)) return "DASH";
        if (this.isHLS(lower)) return this.isHLSMaster(lower) ? "HLS_MASTER" : "HLS_PLAYLIST";
        if (this.isMSS(lower)) return "MSS";
    }
}
(async () => {
    ...
    this.addEventListener('load', async function() {
        if (this._method === "GET") {
            ...
            switch (this.responseType) {
                case "":
                case "text": body = this.responseText ?? this.response; break;
                case "json": body = JSON.stringify(this.response); break;
                case "arraybuffer":
                    ...
                    break;
                case "blob": body = await this.response.text(); break;
            }
            if (body) {
